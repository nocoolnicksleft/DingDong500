...
#fuses XT,NOWDT,NOPROTECT,PUT,NOBROWNOUT,LVP
...
void timeproc()
{
   set_timer1(get_timer1() + TIMER1_10MSEC_20MHZ); // 10msec at 20MHz
   dcfdeltatimer++;
   timeout10msec = 1;
...
void dcf_down()
{
  ...
  if ((dcfdeltatimer > 12) && (dcfdeltatimer<24)) {
   if (!dcfdecodenow) {
     bit_set(dcfbuffer[(dcfbitcount >> 3)],(dcfbitcount & 0x7)); // short for div/mod by 8
   }
  }
...
#define VFD_MODE_HORIZ_VERT 0x40
#define VFD_CMD_GRAPHIC_WRITE 0x18
#define VFD_CMS_SET_MODE 0x1A
...
void vfd_setgraphics_macro(BYTE id, BYTE length, BYTE data[])
{
  BYTE iy;
  vfd_putc(0x1B);
  vfd_putc(0x55);
  ...
  vfd_putc(id);
  vfd_putc(length+4);
  vfd_putc(VFD_CMS_SET_MODE);
  vfd_putc(VFD_MODE_HORIZ_VERT);
    ...
    if (nack) {
     ...
     if (nack) {
	  ...
	  if (timestamp->second != time_sec2) {
       ...
       timestamp->weekday = (time_weekdaymonth >> 5);  
...
void mp3sendcommand()
{
  ...
  else if (mp3status ==  MP3_CMD_QUERY_TITLE) fprintf(mp3player,"FC R 1 %u %lu\r",MP3_COLUMN_LENGTH,((long)75*cfg_mp3title) + (long)30);
  else if (mp3status ==  MP3_CMD_QUERY_ARTIST) fprintf(mp3player,"FC R 1 %u %lu\r",MP3_COLUMN_LENGTH,((long)75*cfg_mp3title));
...
// printf(term,"-buf %X %X %X %X\r\n",dcfbuffer[0],dcfbuffer[1],dcfbuffer[2],dcfbuffer[3]);
   ...
   dcf->month = bcddecode( ((dcfbuffer[6] & 0b00000011) << 3) | ((dcfbuffer[5] & 0b11100000) >> 5) );
...
void dcfsavetime()
{
     ...
     // also setzen und speichern!
      ...
      CurrentTime.Weekday = dcf2.wday-1;  //dcf-wochentag = 1-7
      CurrentTime.Month = dcf2.month;
      CurrentTime.Year = dcf2.year%4;
      CurrentTime.updated = 1;
...
void dcfdecode()
{
 ...
 //      dcfsyncstate = 2;
     ...
     memset(dcfbuffer,0,sizeof(dcfbuffer)); // puffer löschen
}
//**********************************************************
// GET TIME FROM RTC AND DISPLAY TO VFD
//**********************************************************
void do_time() {
...
//  vfd_putc(VFD_CMD_PIXEL_CLEAR ^ dcfsync);
  ...
  else vfd_docmd4(VFD_CMD4_AREA_CLEAR,64,4,71,14);
  ...
  if (cfg_showdebug) {
   vfd_putc(VFD_CMD_FONT_MINI);
   vfd_setcursor(0,22);
   printf(vfd_putc,"%02u %02u %03u %04lu %04lu",dcfbitcount,dcf2.month,cfg_mp3volume,cfg_mp3title,mp3lasttitle);
  }
  ...
  output_bit(ALARMSTATE_LED,cfg_alarmstate);
  output_bit(MP3STATE_LED,cfg_mp3state);
}
...
void mp3displaytime(int8 xpos, int1 running)
{
  ...
  if (running) {
   ...
   vfd_docmd4(VFD_CMD4_AREA_SET,32,60,32+(mp3timebuffer/(mp3totaltime/64)),63);
  } else {
   mp3buffer[mp3bufferptr] = 0;
   mp3timebuffer = atol(mp3buffer + 1);
   mp3totaltime = mp3timebuffer;
  }
...
{
   char c;
   while (c = mp3buffer[start++]) vfd_putc(c);
}
  ...
  char key;
    ...
    if(key!=0) {              // Keypad Control
      ...
      if (!terminalstate) {		// nichts aktiv 
       ...
       if (key == '5') {
        if (cfg_mp3volume > MP3_VOLUME_MAXIMUM) cfg_mp3volume--;
        write_eeprom(EE_MP3VOLUME,cfg_mp3volume);
        mp3docommand(MP3_CMD_SET_VOLUME);
       }
       else
       if (key == '4') {
        if (cfg_mp3volume < MP3_VOLUME_MINIMUM) cfg_mp3volume++;
        write_eeprom(EE_MP3VOLUME,cfg_mp3volume);
        mp3docommand(MP3_CMD_SET_VOLUME);
       }
       else
  ...
  //   fprintf(terminal,"R:%S!",mp3buffer); // echo to terminal
      ...
      if (mp3buffer[0] == 'E') { // fehlermeldung!!
       ...
       if (mp3status == MP3_CMD_QUERY_LENGTH) {
         mp3displaytime(0,0);
         vfd_docmd4(VFD_CMD4_OUTLINE_SET,31,59,97,64);
       }
...
//write_eeprom(EE_MAGICNUMBER,0x0);
 ...
 // set_tris_c(0b11011110);
  ...
  printinfovfd();
  vfd_setgraphics_macro(0x1,8,turm);
  ...
  mp3docommand(MP3_CMD_SET_SERIAL_SPEED);
 ...
 /**********************************************************/
  for (;;) {
    ...
    if (mp3completed == MP3_CMD_SET_SERIAL_SPEED) {
    ...
    if (cfg_mp3state) {
     ...
     if (mp3playing) mp3docommand(MP3_CMD_STOP);
    }
