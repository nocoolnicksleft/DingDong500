...
{
  dcfsignal = 1;
  if (dcfdeltatimer > 90) { // 77
   if (dcfpulsetimeup > 180) { // 130
    dcfdecodenow = 1;
...
void dcf_down()
{
  ...
  if ((dcfdeltatimer > 12) && (dcfdeltatimer<24)) {
   if ((!dcfdecodenow) && (dcfbitcount < 60)) {
...
void printupdatevfd()
{
  ...
  vfd_putc(VFD_CMD_FONT_MINI);
  vfd_setcursor(40,22);
  printf(vfd_putc,"%u %02u %03u %02u %04lu / %04lu",dcfsyncstate,dcfbitcount,dcfpulsetimeup,dcfpulsetimedown, cfg_mp3title,mp3lasttitle);
