...
//int1 dcfignoredown = 0;
int8 dcfsyncstate = 0;
//int16 dcfpulsetimeup = 0;
//int16 dcfpulsetimedown = 0;
//int16 dcfpulsetime;

int8 dcfpulsetimer = 0;
int8 dcfperiodtimer = 0;
int8 pulse;
int8 period;
...
void dcf_up()
{
  ...
  if (dcfpulsetimeup > 78) {
   dcfpulsetime = 0;
   dcfignoredown = 0;
   if (dcfpulsetimeup > 130) {
    dcfdecodenow = 1;
   } // if (dcfpulsetimeup > 130)
  } else dcfignoredown = 1;// if (dcfpulsetimeup > 80)
}
...
{
  dcfpulsetimer = 0;
  if (dcfperiodtimer >= 195) dcfdecodenow = 1;
}
...
void dcf_down()
{
  ...
  int1 pulse1 = 0;
  pulse = dcfpulsetimer;
  period = dcfperiodtimer;
  pulse0 = ( (pulse >  6) && (pulse < 12) );
  pulse1 = ( (pulse > 16) && (pulse < 22) );
  if (pulse0 || pulse1) {
   dcfperiodtimer = pulse;
   if ( (period == 0) || ( (period >= 107) && (period <= 120)) ) {
    if (!dcfdecodenow) {
     if (dcfbitcount < 59) {
      if (pulse1) bit_set(dcfbuffer[dcfbitcount/8],dcfbitcount%8);
     }
     dcfbitcount++;
     if (!dcfsyncstate) dcfsyncstate = 1;
    }
...
void printmenuvfd()
{
  clearcenter();
  vfd_setcursor(0,24);
  printf(vfd_putc,Menu1);
  vfd_setcursor(0,32);
  printf(vfd_putc,Menu2);
  vfd_setcursor(0,40);
...
void printupdatevfd()
{
  if (terminalstate != TSTATE_EDIT_ALARM) {
   vfd_putc(VFD_CMD_FONT_5X7);
   vfd_setcursor(34,7);
  ...
  vfd_putc(VFD_CMD_FONT_MINI);
  vfd_setcursor(45,22);
  printf(vfd_putc,"%u %02u %03u %03u %04lu / %04lu",dcfsyncstate,dcfbitcount,pulse,period, cfg_mp3title,mp3lasttitle);
...
{
 vfd_docmd4(VFD_CMD4_OUTLINE_CLEAR,42,33,84,34);
 vfd_docmd4(VFD_CMD4_OUTLINE_SET,42 + (editorposition * 6),33,48 + (editorposition * 6),34);
}
      ...
      if (!terminalstate) {		// nichts aktiv 
       ...
       if (key == '#') { // escape, nichts ändern, nur raus
        ...
        if ( (key - '0') <= numbers[editorposition]) {
         ...
         vfd_setcursor(42+(editorposition*6),32);
         vfd_putc(key);
