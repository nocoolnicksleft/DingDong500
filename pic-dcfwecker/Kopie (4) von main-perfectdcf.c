...
//  output_high(PIN_A0);
  ...
  if ((dcfdeltatimer > 12) && (dcfdeltatimer<24)) {
   if (!dcfdecodenow) {
     bit_set(dcfbuffer[dcfbitcount/8],dcfbitcount%8);
   }
  }
...
{
  dcfsignal = 1;
  if ( ((dcfdeltatimer > 90) && (dcfdeltatimer < 110))  || 
       ((dcfdeltatimer > 190) && (dcfdeltatimer < 210)) ) {
   if (dcfdeltatimer > 190) {
    dcfdecodenow = 1;
   } 
   dcfvalid = 1;
   dcfdeltatimer = 0;
...
void dcf_down()
{
  ...
  if (dcfvalid) {
   dcfvalid = 0;
   output_high(PIN_A0);
   if (dcfbitcount > 60) dcfbitcount = 0;
   else dcfbitcount++;
  }

  if ((dcfdeltatimer > 12) && (dcfdeltatimer<24)) {
   if (!dcfdecodenow) {
     bit_set(dcfbuffer[(dcfbitcount-1)/8],(dcfbitcount-1)%8);
   }
  }
...
void dcf_up()
{
  ...
  if (dcfpulsetimeup > 77) { 
   dcfpulsetime = 0;
   dcfdeltatimer = 0;
   output_low(PIN_A0);
   if (dcfpulsetimeup > 130) {
    dcfdecodenow = 1;
   } // if (dcfpulsetimeup > 190)
  } // if (dcfpulsetimeup > 97)
}
...
void dcf_down()
{
  ...
  if (dcfpulsetimeup > 77) { 
   ...
   if (!dcfdecodenow) {
     if (dcfbitcount < 59) {
      if (dcfpulsetimedown > 12) bit_set(dcfbuffer[dcfbitcount/8],dcfbitcount%8);
     }
     dcfbitcount++;
...
void printupdatevfd()
{
  ...
  vfd_setcursor(58,14);
  printf(vfd_putc,"%02u",dcfbitcount);

  vfd_setcursor(0,22);
  printf(vfd_putc,"%03u %04lu %04lu ",cfg_mp3volume,cfg_mp3title,mp3lasttitle);
...
void doserial()
{
  char key;
    ...
    if (bkbhit) {
     key = bgetc();
     fprintf(terminal,"%c",key); // echo to terminal
